...
hfi <- DATA$HFI
```
#### Estimating the intensity of bear occurrences based on elevation

```{r}
# intensity as a function of elev
...
#### Computing and plotting residuals of the point process model fitted to elevation data.  Using Pearson residuals and removing any non-finite residuals 
...
#### Creating a point pattern object for each season
...
#### Calculating and plotting the KDE for bear occurrences for each season

```{r}
# KDE Surface Comparison
...
#### Fitting an inhomogeneous Poisson ppm for each season, using elevation. Model used to predict intensity and plotted for each season.

```{r}
# inhomogeneous poission model
for (s in names(ppp_list)) {
  model <- ppm(ppp_list[[s]] ~ elev, covariates = list(elev = elevation))
  plot(predict(model), main = paste("Intensity ~ Elevation -", s))
}
```

#### Fitting a multivariate Poisson Point Process model to bear occurrences for each season, considering all covariates: elevation, forest cover, hfi, and distance to water. Predicted intensity plotted for each season.
...
for (s in names(ppp_list)) {
  ...
  print(summary(model))
  plot(predict(model), main = paste("Predicted Intensity -", s))
}
```
...
for (s in names(ppp_list)) {
  # Compute Ripley's K for the current season
  K <- Kest(ppp_list[[s]])
